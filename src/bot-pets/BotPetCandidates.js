...
function getTop4CandidatesForPlayer(piece) {
    // ══════════════════════════════════════════════════════════════════
    // RUNTIME BOARD VERIFICATION: Verify which Map getTop4Candidates reads
    // ══════════════════════════════════════════════════════════════════
    const runtimeSameMap = (typeof window !== 'undefined' && typeof window.infiniteMap !== 'undefined' 
        && typeof GameState !== 'undefined' && GameState.board && GameState.board.infiniteMap)
        ? (window.infiniteMap === GameState.board.infiniteMap)
        : 'undefined';
    
    const runtimeGlobalSize = typeof window !== 'undefined' && typeof window.infiniteMap !== 'undefined' 
        ? window.infiniteMap.size 
        : 'undefined';
    
    const runtimeGameStateSize = typeof GameState !== 'undefined' && GameState.board && GameState.board.infiniteMap 
        ? GameState.board.infiniteMap.size 
        : 'undefined';
    
    // Verify getCell reads from the correct map by checking sample cells
    const runtimeSampleCells = {};
    if (typeof getCell === 'function') {
        runtimeSampleCells['5,6'] = getCell(5, 6);
        runtimeSampleCells['5,7'] = getCell(5, 7);
        runtimeSampleCells['5,8'] = getCell(5, 8);
    }
    
    console.log('[PET-ONLINE-CANDIDATE-RUNTIME]', {
        sameMap: runtimeSameMap,
        globalSize: runtimeGlobalSize,
        gameStateSize: runtimeGameStateSize,
        sampleCells: runtimeSampleCells,
        piece: piece
    });
...
function refreshBotPetCandidatesForCurrentTurn() {
    ...
    console.log('[PET-ONLINE-TRACE] 1 Firebase board update received');

    // ══════════════════════════════════════════════════════════════════
    // PET-ONLINE-BOARD-AUDIT: Verify both reference AND data
    // ══════════════════════════════════════════════════════════════════
    const sameMap = (typeof window !== 'undefined' && typeof window.infiniteMap !== 'undefined' 
        && typeof GameState !== 'undefined' && GameState.board && GameState.board.infiniteMap)
        ? (window.infiniteMap === GameState.board.infiniteMap)
        : 'undefined';
    
    const globalSize = typeof window !== 'undefined' && typeof window.infiniteMap !== 'undefined' 
        ? window.infiniteMap.size 
        : 'undefined';
    
    const gameStateSize = typeof GameState !== 'undefined' && GameState.board && GameState.board.infiniteMap 
        ? GameState.board.infiniteMap.size 
        : 'undefined';
    
    // Sample actual cells to verify data consistency
    const sampleCells = {};
    if (typeof getCell === 'function') {
        sampleCells['5,6'] = getCell(5, 6);
        sampleCells['5,7'] = getCell(5, 7);
        sampleCells['5,8'] = getCell(5, 8);
    }
    
    console.log('[PET-ONLINE-TRACE] 2 board restore complete');
    console.log('[PET-ONLINE-BOARD-AUDIT]', {
        sameMap: sameMap,
        globalSize: globalSize,
        gameStateSize: gameStateSize,
    ...
    console.log('[PET-ONLINE-TRACE] 3 sameMap=' + sameMap);
    
    // ══════════════════════════════════════════════════════════════════
    // PET-ONLINE-PLAYER: Verify X/O mapping
    // ══════════════════════════════════════════════════════════════════
    const humanPiece = typeof window !== 'undefined' && typeof window.humanPiece !== 'undefined' 
        ? window.humanPiece 
        : 'undefined';
    
    const localPiece = _getLocalPiece();
    
    console.log('[PET-ONLINE-PLAYER]', {
        myOnlineRole: myRole,
        humanPiece: humanPiece,
        localPiece: localPiece,
        currentTurn: turn,
        localTurn: localTurn,
        isOnline: online,
        usesMyOnlineRole: online && localPiece === myRole
    });
    
    console.log('[PET-ONLINE-TRACE] 4 currentTurn=' + turn);
    console.log('[PET-ONLINE-TRACE] 5 localPiece=' + localPiece);
    console.log('[PET-ONLINE-TRACE] 6 localTurn=' + localTurn);
    ...
    if (!localPiece) return _block('no-local-piece');

    console.log('[PET-ONLINE-TRACE] 7 board render complete');
    console.log('[PET-ONLINE-TRACE] 8 getTop4Candidates() called');

    const newCandidates = getTop4CandidatesForPlayer(localPiece);

    console.log('[PET-ONLINE-TRACE] 9 candidateCount=' + newCandidates.length);
    console.log('[PET-ONLINE-TRACE] PASS →', {
        piece: localPiece, count: newCandidates.length,
    ...
    console.log('[PET-ONLINE-TRACE] 10 candidate render complete');
}
...
// ══════════════════════════════════════════════════════════════════
// Online-specific diagnostic to verify TOP-4 candidate state
// ══════════════════════════════════════════════════════════════════
window.debugOnlinePetCandidates = function () {
    const online    = _isOnline();
    const petActive = typeof isBotPetActive === 'function' ? isBotPetActive() : false;
    const myRole    = window.myOnlineRole || 'undef';
    const turn      = typeof currentTurn !== 'undefined' ? currentTurn : 'undef';
    const localTurn = _isMyTurn();
    ...
    console.log('[DEBUG-ONLINE-PET-CANDIDATES]', result);
    return result;
};
