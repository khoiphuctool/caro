...
// ══════════════════════════════════════════════════════════════════
// 🔀 SWITCH TAB KÊNH PHÒNG (Thường / VIP)
// ══════════════════════════════════════════════════════════════════
function switchRoomTab(tab) {
    ...
    if (tabNormal && tabVip) {
        if (tab === 'normal') {
            tabNormal.style.background = '#6366f1'; tabNormal.style.color = 'white';
            tabVip.style.background    = '#e5e7eb'; tabVip.style.color    = '#374151';
        } else {
            tabVip.style.background    = '#f59e0b'; tabVip.style.color    = 'white';
            tabNormal.style.background = '#e5e7eb'; tabNormal.style.color = '#374151';
        }
    }
    // Render ngay từ dữ liệu hiện có (không cần tạo listener mới)
    renderRoomListImmediate();
}
window.switchRoomTab = switchRoomTab;

// Render danh sách phòng ngay lập tức theo tab đang chọn
    ...
    db.ref('rooms').once('value').then(snap => {
        ...
        if (currentRoomTab === 'normal') {
            for (let i = 1; i <= TOTAL_NORMAL_ROOMS; i++) {
                ...
                container.appendChild(_buildRoomEl(roomId, room, i, false, myId));
            }
        } else {
            // VIP: phòng 21-40
            for (let i = 1; i <= TOTAL_VIP_ROOMS; i++) {
                ...
                container.appendChild(_buildRoomEl(roomId, room, roomNum, true, myId));
            }
        }
    });
}

// Helper tạo DOM element cho 1 phòng
function _buildRoomEl(roomId, room, displayNum, isVip, myId) {
    ...
    let statusTxt = '', bgColor = isVip ? '#fffbeb' : '#f8f9fa';
    let borderColor = isVip ? '#f59e0b' : '#ddd', nutHtml = '';

    const btnEnter = isVip
        ? `<button onclick="ngoimVaoPhong('${roomId}')" style="padding:6px 14px;background:#f59e0b;color:white;border:none;border-radius:4px;cursor:pointer;font-weight:bold;">💎 Vào Phòng</button>`
        : `<button onclick="ngoimVaoPhong('${roomId}')" style="padding:6px 14px;background:#28a745;color:white;border:none;border-radius:4px;cursor:pointer;font-weight:bold;">Vào Phòng</button>`;

    if (room.status === 'empty' || !room.status || room.status === 'ended') {
        statusTxt = '🟢 Trống'; nutHtml = btnEnter;
    } else if (room.status === 'waiting') {
        bgColor = isVip ? '#fef3c7' : '#e8f5e9'; borderColor = isVip ? '#d97706' : '#4caf50';
        statusTxt = '⏳ Chờ bắt đầu';
        if (laTrongPhong) {
            const c = isVip ? '#d97706' : '#4caf50';
            nutHtml = `<button onclick="vaoLaiPhong('${roomId}')" style="padding:6px 14px;background:${c};color:white;border:none;border-radius:4px;cursor:pointer;">Vào lại</button>`;
        } else if (!room.playerO_id) {
            const c = isVip ? '#f59e0b' : '#007bff';
            nutHtml = `<button onclick="ngoimVaoPhong('${roomId}')" style="padding:6px 14px;background:${c};color:white;border:none;border-radius:4px;cursor:pointer;font-weight:bold;">Vào Solo</button>`;
        } else {
            nutHtml = `<span style="color:#aaa;font-size:12px;">Đầy</span>`;
        }
    } else if (room.status === 'playing') {
        bgColor = isVip ? '#fed7aa' : '#fff3e0'; borderColor = isVip ? '#ea580c' : '#ff9800';
        statusTxt = '⚔️ Đang chơi';
        if (laTrongPhong) {
            const c = isVip ? '#ea580c' : '#ff9800';
            nutHtml = `<button onclick="vaoLaiPhong('${roomId}')" style="padding:6px 14px;background:${c};color:white;border:none;border-radius:4px;cursor:pointer;">🔄 Vào lại</button>`;
        } else {
            const c = isVip ? '#0ea5e9' : '#17a2b8';
            nutHtml = `<button onclick="xemPhong('${roomId}')" style="padding:6px 14px;background:${c};color:white;border:none;border-radius:4px;cursor:pointer;">👁️ Xem</button>`;
        }
    }
    ...
    const titleHtml = isVip
        ? `<div style="font-weight:bold;font-size:14px;color:#d97706;">💎 Phòng VIP ${displayNum - TOTAL_NORMAL_ROOMS}</div>`
        : `<div style="font-weight:bold;font-size:14px;">Phòng ${displayNum}</div>`;
    el.innerHTML = `
        <div>
            ${titleHtml}
