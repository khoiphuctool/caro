...
const ContextBuilder = {
    ...
    resolveRules(options) {
        // Canonical rule resolution: all bot flows and win validation must come from one shared source.
        if (typeof BlockBothEndsAnalyzer !== 'undefined' && typeof BlockBothEndsAnalyzer.resolveRoomRules === 'function') {
            const resolved = BlockBothEndsAnalyzer.resolveRoomRules(
                typeof options.winCount === 'number' ? options.winCount : undefined
            );
            if (resolved && typeof resolved.winCount === 'number') {
                return {
                    winCount: resolved.winCount,
                    chan2Dau: !!resolved.chan2Dau
                };
            }
        }

        // Legacy fallback only if shared analyzer is unavailable.
